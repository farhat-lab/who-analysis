...
_, drug, drug_WHO_abbr, phenos, tier2 = sys.argv
...
print(f"Fitting {'+'.join(tiers)} tiers, {phenos} model with no PCs for {drug}")
no_PCA = get_pvals_no_PCA(drug, drug_WHO_abbr, f"tiers={'+'.join(tiers)}/phenos={phenos}/dropAF_noSyn_unpooled", binary=True, num_bootstrap=1000, alpha=0.05)
no_PCA.to_csv(f"{drug_WHO_abbr}_{phenos}_tiers={'+'.join(tiers)}_no_PCA.csv", index=False)