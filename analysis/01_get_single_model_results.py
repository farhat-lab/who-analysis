...
def add_significance_category(df, drug, model_path):
    ...
    df.loc[(df["mutation"].str.contains('pncA')) & (df["BH_pval"] <= thresh) & (df["Odds_Ratio"] > 1) & (df["Mut_R"] >= 2) & (df["R_PPV"] >= 0.5), col_name] = "Probable Assoc w R"
    df.loc[(df["mutation"].str.contains('pncA')) & (df["BH_pval"] <= thresh) & (df["Odds_Ratio"] < 1) & (df["Mut_S"] >= 2) & (df["S_PPV"] >= 0.5) & (df["R_PPV"] < 0.4) & (df["R_PPV_UB"] < 0.75), col_name] = "Probable Assoc w S"
    ...
    df.loc[(~df["mutation"].str.contains('pncA')) & (df["BH_pval"] <= thresh) & (df["Odds_Ratio"] > 1) & (df["Num_Isolates"] >= 5) & (df["R_PPV_LB"] >= 0.25), col_name] = "Probable Assoc w R"
    df.loc[(~df["mutation"].str.contains('pncA')) & (df["BH_pval"] <= thresh) & (df["Odds_Ratio"] < 1) & (df["Num_Isolates"] >= 5) & (df["S_PPV_LB"] >= 0.25), col_name] = "Probable Assoc w S"
    
    # get additional evidence from the LRT to be classified as Assoc w R/S. Only classify Tier 1 here
    df.loc[(df[col_name] == "Probable Assoc w R") & (df["BH_LRT_pval"] <= thresh) & (df["Tier"] == 1), col_name] = "Assoc w R"
    df.loc[(df[col_name] == "Probable Assoc w S") & (df["BH_LRT_pval"] <= thresh) & (df["Tier"] == 1), col_name] = "Assoc w S"
    ...
    # for silent mutations, use non-FDR p-values to determine neutrality. But use FDR-corrected p-value for checking if the mutation is NOT significant in the regular permutation test
    df.loc[(df["predicted_effect"].isin(silent_lst)) & (df["BH_pval"] > thresh) & ((df["neutral_pval"] <= thresh) | (df["LRT_neutral_pval"] <= thresh)) & (df["Num_Isolates"] >= 5), col_name] = "Neutral"
