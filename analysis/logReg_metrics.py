...
def compute_downselected_logReg_model(out_dir, pval_col, pval_thresh, tiers_lst, het_mode, synonymous):
    '''
    ...
    # keep only variants below the significance threshold of the desired column. Or keep the ones with NaN pvalues because those
    # are tier 1 features in a tier 1 + 2 model
    if len(tiers_lst) == 1:
        assert sum(pd.isnull(model_analysis[pval_col])) == 0
    
    model_analysis = model_analysis.loc[(model_analysis[pval_col] < pval_thresh) | pd.isnull(model_analysis[pval_col])]
...
summary_df = compute_downselected_logReg_model(out_dir, "BH_pval", 0.05, tiers_lst, het_mode, synonymous)
