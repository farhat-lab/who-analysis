...
vcf_dir = "/n/data1/hms/dbmi/farhat/ye12/who/vcfs_freebayes"
...
def get_lineages(vcf_dirs_files):
    ...
    lineages_df = pd.DataFrame(columns=["VCF_Dir", "File_Name", "Coll2014", 'Freschi2020'])
    ...
    for i, single_dir in enumerate(vcf_dirs_files):
        ...
        if not os.path.isfile(fName):
            ...
            proc = subprocess.Popen(f"fast-lineage-caller {fName} --noheader --pass", shell=True, encoding='utf8', stdout=subprocess.PIPE)
            output = proc.communicate()[0]
            
            # order: Isolate, Coll 2014, Freschi 2020......
            lineages_df.loc[i, :] = [vcf_dirs_files[i], os.path.basename(fName), output.split("\t")[1].replace("lineage", ""), output.split("\t")[2]]
...
lineages_df.to_csv("data/lineages_Coll_Freschi.csv", index=False)