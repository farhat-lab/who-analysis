...
model_suffix = '_STM_pheno_covariate'
...
print(f"Data shape before adding pheno covariates: {matrix.shape}")

# get additional phenotypic covariates and add to the input matrix
df_phenos_STM = pd.read_csv(os.path.join(analysis_dir, "Streptomycin", "phenos_binary.csv"), index_col=['sample_id'])
matrix = matrix.merge(df_phenos_STM[['phenotype']], left_index=True, right_index=True, how='inner')

# remove anything that is 0 everywhere
matrix = matrix[matrix.loc[:, matrix.nunique() > 1].columns]
print(f"Data shape after removing variants that are absent from all samples: {matrix.shape}")
assert 'rrs_n.514A>C' in matrix.columns
...
if not os.path.isfile(os.path.join(out_dir, "model_STM_pheno_covariate.sav")):
    if binary:
    ...
    model.fit(X, y)
    pickle.dump(model, open(os.path.join(out_dir, "model_STM_pheno_covariate.sav"), "wb"))
else:
    model = pickle.load(open(os.path.join(out_dir, "model_STM_pheno_covariate.sav"), "rb"))
